...
try:
    ...
    def Nifty50():
        ...
        # string_logo = '<img src=%s>' % tickerData.info['logoUrl']
    ...
    def NASDAQ():
        ...
        st.info(string_summary)
        # string_summary = stock_info.get_analysts_info(symbol)
        string_summary = tickerData.news
        news_links = links_r(string_summary)
        print(string_summary[0])
        if st.button("news"):
            st.markdown(string_summary[0], unsafe_allow_html=True)
        # st.info(news_links)
        if st.button('Go to Streamlit'):
            js = "window.open({})".format(string_summary[0])  # New tab or window
